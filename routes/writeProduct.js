...
router.post('/uploader', multipartMiddleware, function (req, res) {
    ...
    fs.readFile(req.files.upload.path, function (err, data) {
        db.query('SELECT pid FROM product ORDER BY pid DESC LIMIT 1', function (dbErr, rows) {
                    ...
                    if (fsUsermkdirErr) console.error(fsUsermkdirErr);
                        ...
                        mkdirp('./public/uploads/' + req.session.userId + '/' + newProduct, function (fsProductmkdirErr) {
                            if (fsProductmkdirErr) console.error(fsProductmkdirErr);
                                ...
                                const newPath = __dirname + '/../public/uploads/'
                                    + req.session.userId + '/' + newProduct + '/' + req.files.upload.name;
                                fs.writeFile(newPath, data, function (fsWriteErr) {
                                    if (fsWriteErr) console.log({err: fsWriteErr});
                                        ...
                                        html += "    var url     = \"/uploads/" + req.session.userId + '/' + newProduct + '/' + req.files.upload.name + "\";";
